...
test::TestSimpleCube::TestSimpleCube()
{
    ...
    std::shared_ptr<MeshBuffer> meshComponent = s_Renderer->m_ResourceManager->GetMeshBuffer("cube", cubeMeshLayout);
    std::shared_ptr<Shader> shader = s_Renderer->m_ResourceManager->GetShader("basic");
    std::shared_ptr<Texture2D> texture = s_Renderer->m_ResourceManager->GetTexture("cuteDog");

    std::shared_ptr<Material> material = std::make_shared<Material>();
    std::unique_ptr<Transform> transform = std::make_unique<Transform>();

    material->AddTexture(texture);
    s_Renderer->m_ResourceManager->AddMaterial("dogMat", material);
