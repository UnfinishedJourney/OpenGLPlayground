...
namespace graphics {
    ...
    Bitmap EnvMapPreprocessor::LoadTexture(const std::filesystem::path& texturePath) const {
        Bitmap tex;
        tex.LoadFromFile(texturePath.string(), false, true);
        return tex;
    }

    Bitmap EnvMapPreprocessor::ConvertEquirectangularMapToVerticalCross(const Bitmap& b) const {
        if (b.type() != BitmapType::TwoD) return Bitmap();
        int faceSize = b.width() / 4, outW = faceSize * 3, outH = faceSize * 4;
        Bitmap result(outW, outH, b.components(), b.format());
        const std::array<glm::ivec2, 6> offsets = { glm::ivec2(faceSize, faceSize * 3),
        ...
        int clampW = b.width() - 1, clampH = b.height() - 1;
        for (int face = 0; face < 6; ++face)
            for (int i = 0; i < faceSize; ++i)
                for (int j = 0; j < faceSize; ++j) {
                    ...
                    int U1 = clamp(static_cast<int>(std::floor(Uf)), 0, clampW);
                    int V1 = clamp(static_cast<int>(std::floor(Vf)), 0, clampH);
                    int U2 = clamp(U1 + 1, 0, clampW);
                    int V2 = clamp(V1 + 1, 0, clampH);
                    float s = Uf - U1, t = Vf - V1;
                    glm::vec4 color = b.getPixel(U1, V1) * (1 - s) * (1 - t) +
                        b.getPixel(U2, V1) * s * (1 - t) +
                        b.getPixel(U1, V2) * (1 - s) * t +
                        b.getPixel(U2, V2) * s * t;
    ...
    Bitmap EnvMapPreprocessor::ConvertVerticalCrossToCubeMapFaces(const Bitmap& b) const {
        int faceW = b.width() / 3, faceH = b.height() / 4;
    ...
    Bitmap EnvMapPreprocessor::ComputeIrradianceEquirect(const Bitmap& inEquirect, int outW, int outH, int samples) const {
        if (inEquirect.format() != BitmapFormat::Float || inEquirect.data().empty())
            throw std::runtime_error("ComputeIrradianceEquirect: Invalid input.");
        int srcW = inEquirect.width(), srcH = inEquirect.height();
        if (srcW != 2 * srcH)
        ...
        for (int y = 0; y < outH; ++y) {
            float theta1 = static_cast<float>(y) / outH * MathConstants::PI;
            for (int x = 0; x < outW; ++x) {
                float phi1 = static_cast<float>(x) / outW * MathConstants::TWO_PI;
                ...
                for (int i = 0; i < samples; ++i) {
                    ...
                    int x1 = static_cast<int>(std::floor(Xi.x * srcW));
                    int y1 = static_cast<int>(std::floor(Xi.y * srcH));
                    float theta2 = static_cast<float>(y1) / srcH * MathConstants::PI;
                    float phi2 = static_cast<float>(x1) / srcW * MathConstants::TWO_PI;
    ...
    std::vector<Bitmap> EnvMapPreprocessor::ComputePrefilteredCubemap(const Bitmap& inEquirect, int baseFaceSize, int numSamples) const {
        int mipLevels = static_cast<int>(std::floor(std::log2(static_cast<float>(baseFaceSize)))) + 1;
        ...
        for (int mip = 0; mip < mipLevels; ++mip) {
            ...
            float roughness = (mipLevels > 1) ? static_cast<float>(mip) / (mipLevels - 1) : 0.0f;
            for (int face = 0; face < 6; ++face)
