...
namespace Graphics {
    ...
    glm::vec3 SampleEquirectangular(const Bitmap& envMap, const glm::vec3& dir) {
        ...
        int x0 = clamp(static_cast<int>(floor(xf)), 0, envMap.width() - 1);
        int y0 = clamp(static_cast<int>(floor(yf)), 0, envMap.height() - 1);
    ...
    Bitmap EnvMapPreprocessor::LoadTexture(const std::filesystem::path& texturePath) const {
        int w, h, comp;
        float* img = stbi_loadf(texturePath.string().c_str(), &w, &h, &comp, 3);
        if (!img) {
            throw std::runtime_error("Failed to load texture: " + texturePath.string());
        }
        Bitmap tex(w, h, comp, BitmapFormat::Float, img);
        stbi_image_free(img);
        return tex;
    }
        ...
        for (int face = 0; face < 6; ++face) {
            for (int i = 0; i < faceSize; ++i) {
                for (int j = 0; j < faceSize; ++j) {
                    ...
                    int U1 = clamp(static_cast<int>(floor(Uf)), 0, clampW);
                    int V1 = clamp(static_cast<int>(floor(Vf)), 0, clampH);
    ...
    Bitmap EnvMapPreprocessor::ConvertVerticalCrossToCubeMapFaces(const Bitmap& b) const {
        ...
        // Mark as cubemap.
        // (For each face, copy the corresponding region.)
        ...
        // The face ordering is assumed based on your original layout.
        for (int face = 0; face < 6; ++face) {
    ...
    Bitmap EnvMapPreprocessor::ComputeIrradianceEquirect(const Bitmap& inEquirect, int outW, int outH, int samples) const {
        ...
        for (int y = 0; y < outH; ++y) {
            ...
            for (int x = 0; x < outW; ++x) {
                ...
                glm::vec3 V1 = glm::vec3(sin(theta1) * cos(phi1), sin(theta1) * sin(phi1), cos(theta1));
                glm::vec3 color(0.0f);
                float weight = 0.0f;
                for (int i = 0; i < samples; ++i) {
                    glm::vec2 Xi = hammersley2d(i, samples);
                    int x1 = int(floor(Xi.x * srcW));
                    int y1 = int(floor(Xi.y * srcH));
                    float theta2 = float(y1) / srcH * MathConstants::PI;
                    float phi2 = float(x1) / srcW * MathConstants::TWO_PI;
                    glm::vec3 V2 = glm::vec3(sin(theta2) * cos(phi2), sin(theta2) * sin(phi2), cos(theta2));
                    float D = glm::max(glm::dot(V1, V2), 0.0f);
                    if (D > 0.01f) {
                        color += srcData[y1 * srcW + x1] * D;
                        weight += D;
                    }
                }
                dstData[y * outW + x] = color / weight;
            }
        }
        return outBmp;
    }
    ...
    void EnvMapPreprocessor::SaveAsLDR(const Bitmap& image, const std::filesystem::path& outPath) const {
        ...
        for (int i = 0; i < w * h * comp; ++i) {
            float value = glm::clamp(src[i], 0.0f, 1.0f);
    ...
    void EnvMapPreprocessor::SaveFacesToDiskLDR(const Bitmap& cubeMap,
        ...
        if (cubeMap.depth() != 6) {
            Logger::GetLogger()->error("SaveFacesToDiskLDR: expected 6 faces (depth==6), got {}.", cubeMap.depth());
            return;
        }
    ...
    std::vector<Bitmap> EnvMapPreprocessor::ComputePrefilteredCubemap(const Bitmap& inEquirect, int baseFaceSize, int numSamples) const {
        int mipLevels = static_cast<int>(floor(log2(float(baseFaceSize)))) + 1;
            ...
            for (int face = 0; face < 6; ++face) {
                for (int y = 0; y < faceSize; ++y) {
                    for (int x = 0; x < faceSize; ++x) {
                        ...
                        glm::vec3 R;
                        if (face == 0)       R = glm::vec3(1.0f, -v, -u);
                        else if (face == 1)  R = glm::vec3(-1.0f, -v, u);
                        else if (face == 2)  R = glm::vec3(u, 1.0f, v);
                        else if (face == 3)  R = glm::vec3(u, -1.0f, -v);
                        else if (face == 4)  R = glm::vec3(u, -v, 1.0f);
                        else if (face == 5)  R = glm::vec3(-u, -v, -1.0f);
                        R = glm::normalize(R);
                        ...
                        for (int i = 0; i < numSamples; ++i) {
                            ...
                            if (NdotL > 0.0f) {
                                prefilteredColor += SampleEquirectangular(inEquirect, L) * NdotL;
                                totalWeight += NdotL;
                            }
                        }
                        mipCube.setPixel(x, y, face, glm::vec4(prefilteredColor / totalWeight, 1.0f));
                    }
                }
            }
